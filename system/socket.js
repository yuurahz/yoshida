			...
			async value(PATH, returnAsFilename) {
				...
				const data = Buffer.isBuffer(PATH)
					...
					: /^data:.*?\/.*?base64,/i.test(PATH)
						...
						: /^https?:\/\//.test(PATH)
							? await (res = await Function.fetchBuffer(PATH))
							: fs.existsSync(PATH)
			...
			value(jid, text = "", quoted, options) {
				return conn.sendMessage(
					jid,
					{
						...options,
						text,
						mentions: Function.parseMention(text),
					},
					{
						quoted,
						...options,
			...
			async value(jid, medias, options) {
				...
				for (const i in medias) {
					...
					img.message.messageContextInfo = {
						messageSecret: new Uint8Array(Function.randomBytes(32)),
						messageAssociation: {
							associationType: 1,
...
async function serialize(conn, msg, store) {
	...
	if (msg.key) {
		...
		m.participant =
			jidNormalizedUser(m.message?.participant || m.key.participant) ||
			false;
	...
	if (m.isGroup) {
		...
		m.isAdmin =
			m.isGroup &&
			!!m.groupAdmins.find((member) => member.id === m.sender);
		m.isBotAdmin =
			m.isGroup &&
			!!m.groupAdmins.find(
	...
	m.reply = async (text, options = {}) => {
		if (typeof text === "string") {
			...
			return await conn.sendMessage(
				m.chat,
				{ text, mentions: Function.parseMention(text), ...options },
				{
					quoted: m,
					mentions: Function.parseMention(text),
