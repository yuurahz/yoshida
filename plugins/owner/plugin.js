module.exports = {
  ...
  run: async (m, { Func, setting, plugins }) => {
    ...
    if (!pluginFile)
      return m.reply(Func.texted("bold", `Plugin ${pluginName}.js not found.`));
    const fileName = pluginFile.split("/").pop();
    if (setting.pluginDisable.includes(fileName)) {
      if (m.command === "plugdis") {
        return m.reply(
          Func.texted("bold", `Plugin ${fileName} is already disabled.`),
        );
      }
    }
    if (m.command === "plugdis") {
      if (!setting.pluginDisable.includes(fileName)) {
        ...
        m.reply(
          Func.texted("bold", `Plugin ${fileName} successfully disabled.`),
        );
      } else {
        return m.reply(
          Func.texted("bold", `Plugin ${fileName} is already disabled.`),
      ...
      if (setting.pluginDisable.includes(fileName)) {
        ...
        m.reply(
          Func.texted("bold", `Plugin ${fileName} successfully enabled.`),
        );
      } else {
        return m.reply(
