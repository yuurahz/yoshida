module.exports = {
	...
	run: async (m, { Func, conn }) => {
		try {
			...
			if (!input)
				return m.reply(
					Func.texted("bold", `Mention or reply chat target.`)
				);
			let p = await conn.onWhatsApp(input.trim());
			if (!p.exists)
				return m.reply(Func.texted("bold", `Invalid number.`));
			...
			if (command == "+owner") {
				...
				if (owners.includes(number))
					return m.reply(
						Func.texted("bold", `Target is already the owner.`)
					);
				owners.push(number);
				m.reply(
					Func.texted(
						"bold",
						`Successfully added @${number} as owner.`
					)
				);
				...
				if (typeof data == "undefined")
					return m.reply(
						Func.texted("bold", `Can't find user data.`)
					);
				if (!data.premium)
					return m.reply(
						Func.texted("bold", `Not a premium account.`)
					);
				data.premium = false;
				data.expired = 0;
				m.reply(
				...
				if (jid == conn.user.jid)
					return m.reply(Func.texted("bold", `??`));
				conn.updateBlockStatus(jid, "block").then((res) =>
					m.reply(Func.jsonFormat(res))
				);
			} else if (command == "unblock") {
				conn.updateBlockStatus(jid, "unblock").then((res) =>
					m.reply(Func.jsonFormat(res))
				);
			}
		} catch (e) {
